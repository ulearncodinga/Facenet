...
class cameraThread(QThread):
    ...
    def run(self):
        ...
        while self.running and self.cap.isOpened():
            ...
            if success:
                ...
                bytes_per_line = ch * w
                # 使用 tobytes() 复制数据
                qimage = QImage(rgb_img.tobytes(), w, h, bytes_per_line, QImage.Format_RGB888)

                with self.lock:
                    self.latest_frame = qimage.copy()
                    self.latest_features = self.face_features.copy()
                self.send_result_img.emit(qimage)
               # 使用信号传递 frame
               #  self.send_result_img.emit(rgb_img)
                ...
                self.send_face_feature_box.emit(self.face_boxes, self.face_features, qimage)
            else:
                print("读取摄像头失败")
        # 线程结束，释放摄像头资源
        self.cap.release()
    ...
    def get_latest_data(self):
        #主线程调用的安全方法,返回最新一帧和特征
        with self.lock:
            frame_copy = self.latest_frame.copy() if self.latest_frame else None
            features_copy = self.latest_features.copy() if self.latest_features else []
            return frame_copy,features_copy
...
class MySqlThread(QThread):
    ...
    signal_sql_data = pyqtSignal(dict)
    ...
    def __init__(self):
        ...
        try:
            ...
            self.face_database.setDatabaseName("FaceDatabase.db")
        ...
        while self.running:
            ...
            # 判断接收到的命令是否为查询人脸特征
                ...
                try:
                    ...
                    # 创建临时表备份原表数据，先检查临时表是否已存在，若存在则删除
                    self.query.prepare("SHOW TABLES LIKE 'temp_table'")
                    if self.query.next():  # 如果有结果，说明临时表存在
                        sql_drop_if_exist = "DROP TEMPORARY TABLE temp_table"
                        self.query.prepare(sql_drop_if_exist)

                    sql_temp_table = "CREATE TEMPORARY TABLE temp_table AS SELECT * FROM facedatabase"
                    ...
                    # 删除临时表
                    sql_drop_temp_table = "DROP TEMPORARY TABLE temp_table"
                    self.query.prepare(sql_drop_temp_table)
                    self.query.exec_()
...
#     import sys
#     from PyQt5.QtWidgets import QApplication #需要界面就需要创建QApplication
#
#     app = QApplication(sys.argv)
#     cam_thread = cameraThread()  # 直接创建
#     cam_thread.start()
#
#
#     print("尝试创建 MySqlThread 实例...")
#     db_thread = MySqlThread()   # 这会执行 __init__ 方法
#     print("实例创建完成，检查当前目录是否生成了数据库文件。")
#     # 这里不需要启动线程，因为建表在 __init__ 中已经完成
#
#     sys.exit(app.exec_())